...
describe("NeoForgeMetadata", () => {
    ...
    describe("gameName", () => {
        test("always returns 'cosmic-reach'", () => {
            expect(NeoForgeMetadata.from({} as RawNeoForgeMetadata).gameName).toBe("cosmic-reach");
            expect(NeoForgeMetadata.from(RAW_METADATA).gameName).toBe("cosmic-reach");
        });
    });
    ...
    describe("gameVersions", () => {
        ...
        test("returns the same value as the 'cosmic-reach' dependency", () => {
            const metadata = NeoForgeMetadata.from({ dependencies: { "example-mod": [{ modId: "cosmic-reach", versionRange: "[1.16.5,)" }] } } as unknown as RawNeoForgeMetadata);
    ...
    describe("dependencies", () => {
        ...
        test("special dependencies ('neoforge', 'cosmic-reach', 'java') are ignored by default", () => {
            const metadata = NeoForgeMetadata.from(RAW_METADATA);
