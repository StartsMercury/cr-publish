...
import { CrmmGameVersion } from "@/platforms/crmm/crmm-game-version";
import { CrmmLoader } from "@/platforms/crmm/crmm-loader";
import { CrmmUnfeatureMode } from "@/platforms/crmm/crmm-unfeature-mode";
import { CrmmVersion, CrmmVersionInit, CrmmVersionPatch } from "@/platforms/crmm/crmm-version";
import { CrmmProject, CrmmProjectPatch } from "@/platforms/crmm/crmm-project";
import { CRMM_API_URL, CrmmApiClient } from "@/platforms/crmm/crmm-api-client";
...
const DB = Object.freeze({
    ...
    projects: Object.freeze(JSON.parse(
        readFileSync(resolve(__dirname, "../../../content/crmm/projects.json"), "utf8")
    )) as CrmmProject[],

    versions: Object.freeze(JSON.parse(
        readFileSync(resolve(__dirname, "../../../content/crmm/versions.json"), "utf8")
    )) as CrmmVersion[],
});
...
const CRMM_FETCH = createFakeFetch({
    ...
    PATCH: {
        "^\\/project\\/([^/]+)$": ([idOrSlug], { body }) => {
            ...
            const patch = JSON.parse(body as string) as CrmmProjectPatch;
        ...
        "^\\/version\\/([^/]+)$": ([id], { body }) => {
            ...
            const patch = JSON.parse(body as string) as CrmmVersionPatch;
...
describe("CrmmApiClient", () => {
    describe("getLoaders", () => {
        test("returns loaders", async () => {
            const api = new CrmmApiClient({ fetch: CRMM_FETCH, token: "token" });
    ...
    describe("unfeaturePreviousProjectVersions", () => {
        test("unfeatures versions by their project's id", async () => {
            ...
            const map = await api.unfeaturePreviousProjectVersions({ project_id: "gvQqBUqZ" }, CrmmUnfeatureMode.ANY);
        ...
        test("unfeatures versions by their project's slug", async () => {
            ...
            const map = await api.unfeaturePreviousProjectVersions({ project_id: "lithium" }, CrmmUnfeatureMode.ANY);
            const versions = Object.entries(map).filter(([unfeatured]) => unfeatured).map(([_, version]) => version);
        ...
        test("returns an empty record if a project with the specified id doesn't exist", async () => {
            ...
            const map = await api.unfeaturePreviousProjectVersions({ project_id: "QQQQQQQQ" }, CrmmUnfeatureMode.ANY);
        ...
        test("returns an empty record if a project with the specified slug doesn't exist", async () => {
            ...
            const map = await api.unfeaturePreviousProjectVersions({ project_id: "not-a-real-slug" }, CrmmUnfeatureMode.ANY);
